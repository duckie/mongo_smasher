...
// the consumers (db writers) threads. Readers unlocks writers
// first whee freing the lock whereas writers unlocks readers
// first when freing the lock.
// Did not use boost here cause boost's queue does not implement
// move semantics. I could workaround with a container object
// but its more simple to write a reusable queue.
//
template <class T>
class Queue {
 ...
 public:
  ...
  T pop(duration_t* block_duration = nullptr) {
    ...
    if (block_duration) *block_duration = duration_t{0u};
    if (0u == size_) {
      ++waiting_readers_;
      time_point_t start = wait_clock_t::now();

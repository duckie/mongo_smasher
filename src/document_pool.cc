...
DocumentPool::DocumentPool(Randomizer& randomizer, std::string const& db_uri,
                           std::string const& db_name, std::string const& collection_name, update_method method, size_t size,
                           size_t reuse_factor)
    : randomizer_{randomizer},
      ...
      for (;;) {
        ...
        if (thread_command_type::stop == command.type)
          break;
        ...
        if (update_method_ == update_method::latest) {
          ...
          for (auto&& doc_view : cursor) {
            new_documents.emplace_back(Document{0u, std::shared_ptr<document::value>(new document::value(doc_view))});
          }
        }
        ...
        if (0 == new_documents.size()) {
          log(log_level::warning, "Document pool for %s.%s failed to retrieve any documents.", db_name.data(), collection_name.data());
        }
        else {
